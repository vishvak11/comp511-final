...
def train_graph_encoder(
    ...
    for epoch in ticker:
        ...
        for batch in neighbor_loader:
            ...
            optim.zero_grad()
            loss, comps, _ = model(batch_inputs, sampled_edge_index, batch_gene_weights)
            loss = loss.mean().to(device)
        ...
        if epoch % eval_freq == 0:
            ...
            if eval_loss_epoch < best_eval_loss:
                ...
                #torch.save(sd, cachedir / f"model_step{epoch}.pt")
                torch.save(sd, cachedir / f"model.pt")

        # ---- CACHE (epoch-based) ----
        if epoch % cache_freq == 0:
            torch.save(state_dict(model, optim, step=(epoch + 1)), cachedir / f"last.pt")
    ...
    # final safety save
    torch.save(state_dict(model, optim, step=epoch), cachedir / f"last.pt")  